...
def _load_manifest(manifest: Path, display_path: str, problems: list[str]) -> dict | None:
    """Parse and validate one controls.yaml, appending any problems it finds.

    Used for both Terraform modules and non-module components (the scanning
    pipeline) — a control is a control regardless of what implements it, and the
    same evidence discipline applies to each.
    """
    try:
        data = parse_controls_yaml(manifest.read_text(encoding="utf-8"))
    except ValueError as e:
        problems.append(f"{display_path}/controls.yaml: {e}")
        return None

    name = data.get("module") or Path(display_path).name
    ...
    return {"name": name, "path": display_path, **data}


def collect(root: Path, component_files: tuple[str, ...] = ()) -> tuple[list[dict], list[str]]:
    units, problems = [], []
    ...
    for mod_dir in find_modules(root):
        ...
        if not manifest.exists():
            ...
            continue
        unit = _load_manifest(manifest, str(rel), problems)
        if unit is not None:
            units.append(unit)

    # Components declare controls the same way modules do, but are not Terraform
    # (the CI scanning estate, whose controls.yaml lives beside the workflow that
    # orchestrates it). Passed in explicitly so the Terraform-module rule — every
    # .tf directory must have a controls.yaml — is not applied to things that are
    # not Terraform, such as policy test fixtures.
    for cf in component_files:
        path = Path(cf)
        if not path.exists():
            problems.append(f"{cf}: component controls.yaml not found")
            continue
        unit = _load_manifest(path, str(path.parent), problems)
        if unit is not None:
            units.append(unit)
...
def to_markdown(modules: list[dict], report: dict) -> str:
    lines = [
        ...
        (
            ...
            f"**{report['modules']} components**, with **{report['exceptions']} documented "
            "policy exceptions**."
        ),
        "",
...
def main(argv: list[str] | None = None) -> int:
    ...
    ap.add_argument(
        ...
        default=[],
        metavar="PATH",
        help="a controls.yaml for a non-Terraform component (the scanning pipeline); repeatable",
    )
    ...
    modules, problems = collect(args.root, tuple(args.component))
    ...
    if args.check:
        if problems:
            ...
            return 1
        print(
            f"control-mapping check passed · {report['modules']} components · "
